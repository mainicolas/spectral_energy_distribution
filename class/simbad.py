...
class Simbad():
	...
	def get_data(self):
		'''
		update the following attributes from the simbad request:
			self.flux[0] -> filter name
			self.flux[1] -> flux value
			self.flux[2] -> flux error

			self.coordinates[0] -> ra
			self.coordinates[1] -> dec
		'''
	...
	def waveband(self):
		'''
		update the following attributes from the simbad request:
			self.flux_Jy[0] -> flux in jansky
			self.flux_Jy[1] -> error+ in jansky
			self.flux_Jy[2] -> error- in jansky

		return:
			waveband_col (astropy column with information on the filters)
		'''
		waveband = []
	...
	def simbad_table(self):
		'''
		create and print a readable astropy table output 
		'''
		self.get_data()
